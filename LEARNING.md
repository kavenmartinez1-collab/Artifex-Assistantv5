...
4. **Idle shrink** — `model_queue.IDLE_SHRINK_SEC`, default 600. After 10 minutes of no requests, the engine releases its VRAM so other GPU work (other AI loads, games, browsers) can use it. The next request rebuilds at whatever tier it needs. The shrink runs in a lazy-started asyncio task that holds the queue lock while unloading, so it never races a request. Override with `ARTIFEX_IDLE_SHRINK_SEC` (`<= 0` disables it): the timer keys off traffic *through this wrapper*, so a latency-sensitive client that talks to llama-server directly looks idle here and would otherwise get unloaded mid-session.
...
## 21. Browser Vision — One Descriptor, Many Towers

The text engine's hourglass (many weight sources → one ModelDescriptor → one forward pass) had to be rebuilt for vision, because the towers genuinely differ where it counts. The discipline that made it tractable was the same one that built the engine: **a descriptor captures every axis families differ on, and a parity harness against the reference implementation decides when "built" becomes "verified."**

### The cross-family contract

A `VisionDescriptor` is the vision twin of `ModelDescriptor`. Qwen3-VL and Gemma 4 share the *shape* of a ViT — patchify, blocks of (norm → attention → norm → MLP), a projector to text-embedding space — and differ on nearly every detail:

| Axis | Qwen3-VL | Gemma 4 |
|---|---|---|
| Norm | LayerNorm (mean + var + bias) | RMSNorm, no biases |
| QKV | fused, interleaved 2D RoPE (θ 10000) | split, **per-head** Q/K RMS norms, x/y-half RoPE (θ **100**) |
| Attention scale | 1/√d | **1.0** |
| Projector | spatial-merge MLP | 3×3 avg-pool → linear |
| Extras | DeepStack taps into early text layers | QAT clamps around every projection, sandwich post-norms |

The contract encodes all of it; the encoder switches build paths on `towerVariant`. A family's builder ships `verified: false` until it passes parity — and the UI *honors* that flag, refusing or flagging unverified families instead of silently producing embeddings the LLM will treat as noise.

### Parity is the difference between "runs" and "works"

Qwen3-VL's first end-to-end run produced a coherent, detailed description of a screenshot — and was **wrong in a way only a reference comparison could catch**. The `__VISION_PARITY__` harness diffs the browser pipeline against a golden fixture generated by the *real* HuggingFace tower, stage by stage:

1. **Preprocessing**: bit-exact (0.0) on the first try — patch ordering and normalization were right.
2. **Tower output**: mean-abs-diff **0.45** — clearly broken, and the error *grew with depth* (deepstack[0] at block 5 was closest). Depth-growing error fingerprints a small systematic mistake at the input compounding, not a structural bug.

Two reference reads cracked it. First, Qwen3-VL's ViT applies **2D rotary embeddings to Q/K in every block** on top of the learned position table — the encoder had skipped RoPE entirely (position-blind attention that still produced plausible text). Second, the position-embed interpolation used the half-pixel convention; HF uses align-corners (`linspace(0, S-1, grid)`). Both fixes were verified *against the HF functions directly* (1e-7 and 1e-5) before touching the GPU, so the browser numbers afterward were pure kernel behavior: **0.45 → 8e-5**. That's the gap between an answer that sounds right and one that is right.

### Three weight sources, proven by reuse

The payoff of the descriptor: the *same* encoder ran Qwen3-VL from HF safetensors (`model.visual.*`) and Qwen3.6 from a GGUF `mmproj` file (`v.blk.*` clip.cpp names, fused-QKV-via-row-slices, summed temporal conv kernels) with only a locator and a loader between them. The 27B + mmproj then read `BLMM-EN001`, a serial number, and a 1st-edition stamp off a trading card — OCR-grade vision through a GGUF clip tower that had never run in a browser, on a consumer GPU.

Gemma 4 is the honest counterexample still open: its tower (RMS norms, QAT activation clamps, per-head QK norms, θ-100 x/y RoPE, avg-pool projector — every op extracted from llama.cpp's `clip_graph_gemma4v`) is *built* and runs end-to-end, but **confabulates** — it invented a Magic: The Gathering card instead of seeing the one in front of it. Built ≠ verified. Closing it needs its own oracle (the gemma-4 HF weights) and the same stage-by-stage bisection; until then the UI says EXPERIMENTAL and means it.

Lesson: a vision model that produces fluent, specific, *plausible* output is the most dangerous failure mode there is — it passes every check except the one that matters. Only a reference comparison tells you whether the model is reading the image or hallucinating around it.

## 22. Auditing TurboQuant — When "Available Everywhere" Means "Wrong Somewhere"

TurboQuant KV compression was built and validated before the Phase A refactor that made the engine multi-family. The checkbox was *exposed* on every model; the audit asked whether it was *correct* on every model, and the answer was no.

The compressed-KV path reconstructs K/V into **global-kvDim scratch** and attends without the per-layer features the modern engine added: sliding-window masks, KV-sharing (`kvSourceLayer`), and per-layer head dims. On a uniform dense model none of those exist, so it was fine. On Gemma 4 — which uses all three — enabling compression would have silently produced wrong output: the worst kind of bug, because nothing crashes and the text still reads fluently.

The fix follows the house rule (no silently-wrong output): `createKVCache` now **refuses** compression on any model whose layers use those features, naming the offending one, rather than misleading. Supported and now parity-confirmed: uniform full-attention (dense Qwen) and the full-attention layers of DeltaNet hybrids.

The audit also caught a memory bug hiding behind correctness: the compressed allocator sized buffers for **all** layers, including the linear-attention layers of a hybrid that never touch the KV cache — 54 of the 27B's 64. Those layers now get a 4-byte placeholder, and the savings log reports over the real full-attention count.
